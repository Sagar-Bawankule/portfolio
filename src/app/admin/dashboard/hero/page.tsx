...
const emptyHero: HeroData = { name: '', tagline: '', description: 'Available for work', resumeUrl: '/resume.pdf', skills: [], profileImage: '/profilephoto.webp' }

export default function HeroPage() {
    const [formData, setFormData] = useState<HeroData>(emptyHero)
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await (await fetch('/api/hero')).json()
    ...
    return (
        <div className="max-w-3xl">
            <div className="mb-8">
                ...
                <p className="text-slate-600">Edit your hero section content</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 space-y-6">
                ...
                <div><label className="block text-sm font-medium text-slate-700 mb-1">Tagline</label><textarea value={formData.tagline} onChange={(e) => setFormData({ ...formData, tagline: e.target.value })} className="w-full px-4 py-2 border border-gray-200 rounded-lg" rows={2} required placeholder="Full Stack Developer & UI/UX Designer..." /></div>

                <div><label className="block text-sm font-medium text-slate-700 mb-1">Status Description</label><input type="text" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full px-4 py-2 border border-gray-200 rounded-lg" placeholder="Available for work" /></div>

                <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-700 mb-1">Resume URL</label><input type="text" value={formData.resumeUrl} onChange={(e) => setFormData({ ...formData, resumeUrl: e.target.value })} className="w-full px-4 py-2 border border-gray-200 rounded-lg" placeholder="/resume.pdf" /></div>
                ...
                <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Featured Skills</label>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {formData.skills.map((skill, i) => (
                            <span key={i} className="px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm flex items-center gap-1">
                                {skill}<button type="button" onClick={() => setFormData({ ...formData, skills: formData.skills.filter((_, j) => j !== i) })} className="hover:text-purple-900"><X className="w-3 h-3" /></button>
                            </span>
                        ))}
                    </div>
                </div>
